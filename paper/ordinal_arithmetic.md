...
#### The unresolved self-derivation cycle.

The exact native target must be distinguished from a proved theorem. Let $\mathcal A$ be the 38 logical clauses of the reviewed Hypermath translation, with its declared parameters and finite path definitions \[16\]. Write $g$ for ground, $f$ for form application, and $d$ for the deriver parameter. Let $\mathsf{SD}$ be the original four-conjunct target: ground self-continuation, reflexive finite self-reads, the two path-composition identity laws, and $\mathsf{Cyc}:=\operatorname{Simulation}(f(f(d)),d)$.

#### Proposition 6.13 (The missing simulation cycle).

Relative to $\mathcal A$, one has $\mathsf{SD}\leftrightarrow\mathsf{Cyc}$. There are interpretations of all clauses of $\mathcal A$ in which $\mathsf{SD}$ is false and interpretations in which it is true. The clauses also do not entail the nontrivial simulation-pair claim $$\exists x,y\;\bigl(\operatorname{Similar}(x,y)\land
\operatorname{Simulation}(x,y)\land x\ne y\bigr).$$

#### Proof.

Ground self-continuation is a clause of $\mathcal A$; every form has its zero-step self-read; and path composition with the ground path on either side satisfies the stated identity clauses. These establish the first three conjuncts of $\mathsf{SD}$. Its last conjunct gives the forward implication, and adjoining it to the first three gives the reverse implication.

In the two-chain interpretation of Proposition 6.10, $d=g=(0,0)$ and simulation is equality. Thus $f(f(d))=(2,0)\ne(0,0)$, so $\mathsf{Cyc}$ and $\mathsf{SD}$ are false. Simulation as equality also refutes the displayed nontrivial-pair claim. The same interpretation still satisfies every clause of $\mathcal A$.

For the positive interpretation, use the six-form model in `FiniteActionCountermodel.lean` \[16\]. Its forms are $a_0,a_1,a_2,b_0,b_1,b_2$, and application has the chains $a_0\mapsto a_1\mapsto a_2\mapsto a_1$ and $b_0\mapsto b_1\mapsto b_2\mapsto b_0$. Ground is $a_0$, deriver is $a_1$, and simulation is equality. Congruence is equality of the class labels $0,1,2,3,1,2$ respectively; similarity is universal. The remaining interpretations and all 38 clause checks are given in that model. Here $f(f(d))=d$, so $\mathsf{Cyc}$ holds and the first part gives $\mathsf{SD}$.

The equivalence is checked by `selfDerivation_iff_driverCycleClaim`; its conditional assembly uses existing clauses and parameters, with no admitted proof. The three negative results in `FullAxiomModel.lean` have no axiom dependencies. Certifying that the complete model satisfies $\mathcal A$ uses Lean's propositional extensionality, classical choice, and quotient soundness. The models concern the present translation, whose adequacy to the intended source remains open.

The six-form model also refutes the proposed arithmetic bridges discussed above. Thus establishing the cycle would still not prove those bridges. The foundation now retains `selfDerivation` as the original unproved proposition and exposes conditional assembly separately. Five admitted proof sites for the cycle and nontrivial pair were withdrawn; their removal does not prove the claims. A source-adequate simulation and generation account must justify any rule excluding the countermodel. Assuming the desired cycle as an axiom would assume the remaining conjunct. Native ranked acceptance and arithmetic coverage need their own constructions beyond that obligation.
...
Tyler Roost. *Hypermath: Fractal meta-representation and Gödelian completeness research target*. Development source and proof audit at commit [b35b795c13390e5e10430238e033955ca5053847](https://github.com/TimeLordRaps/hypermath/tree/b35b795c13390e5e10430238e033955ca5053847), 2026. The repository records open proof obligations and countermodels; it is not an independent completeness certificate.
