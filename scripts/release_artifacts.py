...
SCHEMA_VERSION = "ORDINATICS-PUBLIC-RELEASE-1.0"
ARCHIVE_STEM = "ordinatics"
DISTRIBUTION_NAME = "ordinatics"
IMPORT_PACKAGE = "ordinatics"
...
def _repository_url(repo: Path) -> str:
    try:
        ...
        return "https://github.com/TimeLordRaps/ordinatics"
...
def _verify_python_distributions(wheel: Path, sdist: Path, release: str) -> None:
    ...
    with zipfile.ZipFile(wheel) as bundle:
        ...
        if len(metadata_paths) != 1:
            raise ReleaseError("wheel must contain one METADATA")
...
def build_source(repo: Path, ref: str, release: str, output_dir: Path) -> tuple[Path, Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="ord-source-archive-") as temporary:
    ...
    manifest = {
        ...
        "release": release,
        "scope": (
            "public Ordinatics library, manuscript sources, examples, tests, "
            "and public project process"
        ),
        "source": {
...
def _build_wheel_once(source_archive: Path, prefix: str, destination: Path, epoch: str) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="ord-wheel-") as temporary:
...
def _build_sdist_once(
    ...
    destination.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="ord-sdist-") as temporary:
        temporary_path = Path(temporary)
        with zipfile.ZipFile(source_archive) as bundle:
...
def build_all(
    ...
    with tempfile.TemporaryDirectory(prefix="ord-wheel-builds-") as temporary:
        temporary_path = Path(temporary)
    ...
    with tempfile.TemporaryDirectory(prefix="ord-sdist-builds-") as temporary:
        temporary_path = Path(temporary)
