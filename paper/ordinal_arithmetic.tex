...
\author{Tyler Roost}
\date{Manuscript draft, September 6, 2026}
\begin{document}
\maketitle
\begin{abstract}
...
\end{abstract}
\noindent\textbf{Keywords:} Ordinatics; ordinal arithmetic; satisfaction; ramified truth; partial specialization; derivation paths.
...
\]
Here $K$ is the field of rational functions. The ring $R$ consists of rational functions admitting a representation with denominator nonzero at $c$. Define
\[
...
The map $W$ is a well-defined surjective unital ring homomorphism on $R$. Its kernel is $(X-c)R$. It cannot extend to a unital field homomorphism $K\to\Q$. For $r\in R$, the inverse $r^{-1}$ belongs to $R$ exactly when $W(r)\ne0$.
\end{theorem}
\begin{proof}
...
Let $L_0=\{0,1,+,\cdot,<\}$ be the first-order language of arithmetic, with equality. Fix a finite-string coding into $\N$ of terms and formulas, including codes for ordinal polynomial indices below $\theta$. The symbol $T_\beta$ is a unary predicate for each $\beta<\theta$. For $\alpha\le\theta$, put
\[
